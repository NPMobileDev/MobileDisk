...
- (BOOL)openFileAndSetupReadSource
{
	...
	dispatch_source_set_event_handler(readSource, ^{
		...
		HTTPLogVerbose(@"%@[%p]: Attempting to read %u bytes from file", THIS_FILE, self, bytesToRead);
		...
		{
			HTTPLogVerbose(@"%@[%p]: Read %zd bytes from file", THIS_FILE, self, result);
...
- (NSData *)readDataOfLength:(NSUInteger)length
{
	...
	if (data)
	{
		...
		HTTPLogVerbose(@"%@[%p]: Returning data of length %u", THIS_FILE, self, dataLength);
